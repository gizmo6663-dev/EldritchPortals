...
''')
    ...
    RULES = [
      ("Basic Rules", "", [
        ("Skill Rolls", [
          ...
          "  Ex: skill 60, Hard difficulty (req 30)",
    "    -> fumble on 96–100",
        ]),
        ("Difficulty Levels", [
    ...
    "  Result ≤ skill = no improvement.",
      ...
      ("Combat", "", [
        ...
        ("Melee", [
          ...
          "  extra dodges cost action next round.",
          "",
          "OUTNUMBERED:",
          ...
          "  can dodge/fight back as many times.",
        ...
        ("Firearms", [
          ...
          "  Medium distance (base range): normal",
    ...
    "Nyctophobia – darkness",
          ...
          "Xenophobia – strangers/unknowns",
          "Zoophobia – animals",
        ]),
        ("Manias (selection)", [
          "Dipsomania – craving alcohol",
    ...
    "Thanatomania – death wish",
      ...
      ("Chase", "", [
        ...
        ("Movement & Actions", [
          ...
          "",
          "Handling hazards costs action AND",
          "  movement that round.",
        ]),
        ("Barriers", [
      ...
      ("Magic & Tomes", "", [
        ...
        ("Mythos Beings (SAN)", [
          "Being: success / failure SAN loss",
          "",
          "  Byakhee: 1/1d6",
          "  Dark Young: 0/1d8",
          "  Deep One: 0/1d6",
      ...
      ("Pulp Cthulhu", "", [
        ("Pulp Rules", [
          ...
          "",
          "Combat rolls CANNOT be pushed (standard).",
    ...
    ("Ranged Weapon Table", [
        ...
        ("Age Effects", [
          ...
          "  40–49: EDU +2, -5 among STR/CON/DEX,",
          "    APP -5, MOV -1",
          "  50–59: EDU +3, -10 among STR/CON/DEX,",
          "    APP -10, MOV -2",
          "  60–69: EDU +4, -20 among STR/CON/DEX,",
          "    APP -15, MOV -3",
          "  70–79: EDU +4, -40 among STR/CON/DEX,",
          "    APP -20, MOV -4",
          "  80–89: EDU +4, -80 among STR/CON/DEX,",
          "    APP -25, MOV -5",
        ]),
        ("Credit Rating", [
    ...
    def request_all_files_access():
        ...
        try:
            ...
            try:
                ...
                # Fallback: generic "All files access" screen
                intent = Intent(
    ...
    class FilePicker:
        ...
        Android versions, and the user can pick from anywhere
        ...
        def _on_result(self, request_code, result_code, intent):
            """Received result from file picker."""
    ...
    class EldritchApp(App):
        ...
        # ---------- COMBAT (Initiative + Map in sub-tabs) ----------
        def _mk_combat(self):
        ...
        def _cmb_render(self):
            ...
            # Point init-tracker target to cmb_area
        ...
        def _mk_cmb_map(self):
            ...
            if n_enemy:
                summary.append(f"{n_enemy} enemy(ies)")
            if n_s:
        ...
        def _mk_tool(self):
            ...
            self._scen_init()
            # Migrate away from old 'init' sub-tab if it remains
            ...
            # should use this tool_area — but since init sub-tab is removed,
            # it should not happen. Set target to None for safety.
        ...
        def _chars_do_pick_file(self):
            ...
            self._chars_show_message(
                ...
                "Documents, Downloads, Drive, or wherever "
                "you have the file.",
                is_error=False)
            Clock.schedule_once(
        ...
        def _chars_normalize_entry(self, entry):
            ...
            if raw_type.lower() == 'pc':
                ...
                result['type'] = 'NPC'
            elif raw_type.lower() in ('enemy', 'fiend', 'foe',
        ...
        def _chars_show_import_preview(self, chars, skipped):
            ...
            skip_text = (
                f"\n\n({skipped} entry"
                f"{'ies' if skipped != 1 else ''} without names were "
                "skipped)"
            ) if skipped else ""
        ...
        def _init_tracker_init(self):
            """Initialise state for initiative tracker."""
            if not hasattr(self, '_init_phase'):
                self._init_phase = 'setup'
                self._init_list = []
            # target_area is overridden by Combat tab to self._cmb_area
